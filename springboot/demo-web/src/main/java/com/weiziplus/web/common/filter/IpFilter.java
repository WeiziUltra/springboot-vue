package com.weiziplus.web.common.filter;
...
public class IpFilter extends BaseService implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        //获取白名单列表
        Set<String> ipValueListWhite = dataDictionaryIpManagerService.getWebIpFilterWhiteList();
        //如果当前ip是白名单---直接放过
        ...
        //获取ip过滤规则
        String ipFilterRole = dataDictionaryIpManagerService.getWebIpFilterRole();
        //只允许白名单
        ...
        if (DataDictionaryIpManagerService.IpFilterRole.BLACK.getValue().equals(ipFilterRole)) {
            Set<String> ipFilterBlackList = dataDictionaryIpManagerService.getWebIpFilterBlackList();
        ...
        if (number >= MAX_NUM) {
            ...
            dataDictionaryIpManagerService.handleWebAbnormalIp(ipAddress);
            response.setStatus(HttpServletResponse.SC_FORBIDDEN);
            response.getWriter().print(JSON.toJSONString(ResultUtils.errorRole("访问频率过快,请稍后重试")));
            return;
        }
